...
This is the hardest benchmark. While accuracy dropped slightly to **86.91%**, the **ROC AUC remains very high (0.9385)**, suggesting the model is highly sensitive to spoofing even in unseen categories.
...
*   **Precision vs. Recall:** High F1-scores and PR AUC values (all > 0.99) confirm the model is robust against class imbalance, maintaining high detection rates for both Bonafide (Real) and Spoof (Fake) samples even in skewed datasets.
*   **ROC AUC Robustness:** The consistent ROC AUC above **0.93** across all benchmarks proves the model has high discriminative power regardless of the decision threshold chosen.
